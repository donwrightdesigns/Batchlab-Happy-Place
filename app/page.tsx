...
export default function REBEPage() {
  ...
  const [prompt, setPrompt] = useState('Enhance lighting, add a vibrant blue sky, and improve curb appeal with lush green grass and clean driveway.');
  const [resolution, setResolution] = useState<ImageResolution>('2K');
  const [aspectRatio, setAspectRatio] = useState<any>('1:1');
  const [model, setModel] = useState<ImageModel>('basic');
  const [suffix, setSuffix] = useState('_beautified');
  ...
  const [creativeStrength, setCreativeStrength] = useState(0.7);
  ...
  const [systemInstruction, setSystemInstruction] = useState("You are a technically savvy, OCD creative director for high-end real estate photography. Your goal is to achieve a 'DAYLIGHT AIRY DIFFUSED NATURAL' style that looks professionally edited but remains grounded in reality. \n\nMaintain the exact structural integrity and perspective of the 'TARGET IMAGE'. Use 'SPATIAL CONTEXT' images only to understand the room's geometry and light sources. Use 'STYLE REFERENCE' images only if explicitly requested for aesthetic cues. Return ONLY the edited image data.");
  ...
  const getModelDisplay = () => {
    if (model === 'basic') return { icon: '⚡', label: 'Flash 2.5' };
    if (model === 'nano-2') return { icon: '🍌🍌', label: 'Nano 2' };
    return { icon: '🎯', label: 'Pro' };
  };

  const cycleModel = () => {
    if (model === 'basic') setModel('nano-2');
    else if (model === 'nano-2') setModel('pro');
    else setModel('basic');
  };
  ...
  const goToNextImage = useCallback(() => {
    ...
    openImageDetail(images[nextIndex]);
  }, [selectedImage, images, openImageDetail]);

  const goToPreviousImage = useCallback(() => {
    if (!selectedImage) return;
    const currentIndex = images.findIndex(img => img.id === selectedImage.id);
    const prevIndex = (currentIndex - 1 + images.length) % images.length;
    openImageDetail(images[prevIndex]);
  }, [selectedImage, images, openImageDetail]);
  ...
  }, [selectedImage, isAnalyzing, fileToBase64]);
  ...
  const processBatch = useCallback(async () => {
    ...
    const processImage = async (img: ImageFile) => {
      ...
      await saveToMemory({
        ...
        settings: { prompt, resolution, model, aspectRatio, creativeStrength }
      });
    ...
    if (batchImages.length > 0) {
      ...
      for (const img of batchImages.slice(0, 4)) {
        const memoryMatch = localMemory.find(m => m.id === img.id);
    ...
    setIsProcessing(false);
  }, [images, isProcessing, prompt, resolution, model, aspectRatio, contextFiles, fileToBase64, concurrencyLimit, systemInstruction, isBatchMode, creativeStrength, useContextForStyle, propertyAddress, tier, addNetworkLog, updateNetworkLog]);

  const reprocessSingle = useCallback(async () => {
    ...
    setIsProcessing(false);
  }, [selectedImage, isProcessing, reprocessPrompt, resolution, model, aspectRatio, fileToBase64, systemInstruction, creativeStrength, contextFiles, useContextForStyle, tier]);
  ...
  const downloadAll = async () => {
    ...
    saveAs(content, 'beautified_real_estate_photos.zip');
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F8F9FA]">
          ...
          <div className="flex items-center gap-2">
            ...
            {user && (
              <div className="hidden sm:flex items-center gap-2 px-3 py-2 bg-white/5 rounded-lg border border-white/10">
                <div className={`w-1.5 h-1.5 rounded-full ${isDriveLinked ? 'bg-green-500 shadow-[0_0_8px_#22c55e]' : 'bg-gray-500 animate-pulse'}`} />
                <span className="text-[9px] font-bold text-white uppercase tracking-widest">
                  {isDriveLinked ? 'Drive' : 'Drive?'}
                </span>
              </div>
            )}
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        <div className="w-full lg:w-80 border-r border-border bg-white p-6 flex flex-col gap-6 overflow-y-auto">
          <section>
            ...
            <div className="flex flex-wrap items-center gap-1.5 mb-3">
              <button 
                onClick={cycleModel}
                className="flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-wider bg-gray-100 hover:bg-gray-200 text-gray-700 px-2.5 py-1 rounded-md transition-colors"
                title="Click to cycle AI Model"
              >
                <span>{getModelDisplay().icon}</span>
                <span>{getModelDisplay().label}</span>
              </button>
        ...
        {/* Right Panel: Gallery */}
        <div className="flex-1 bg-bg p-4 overflow-y-auto relative">
                  ...
                  >
                    <div className="aspect-[4/3] relative bg-gray-200 cursor-pointer overflow-hidden rounded-md">
                      ...
                      {img.status === 'completed' && (
                        <div className="absolute top-2 left-2 bg-green-500 text-white p-1 rounded-full shadow-lg pointer-events-none">
      ...
      <AnimatePresence>
        {showMemory && (
          <>
            ...
            >
              <div className="p-4 sm:p-8 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between bg-white sticky top-0 z-10 gap-4">
                <div className="flex items-center justify-between w-full sm:w-auto">
                  <div>
                    <h2 className="font-display font-black text-2xl sm:text-3xl tracking-tighter uppercase leading-none">YOUR BATCHES</h2>
                    <p className="text-[10px] font-bold text-gray-400 mt-2 uppercase tracking-[0.2em]">Archived Enhancement Sessions</p>
                  </div>
                  <button onClick={() => setShowMemory(false)} className="p-2 hover:bg-gray-100 rounded-full transition-colors border border-gray-200 sm:hidden">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                  {getFolderId() && (
                    <a 
                      href={`https://drive.google.com/drive/folders/${getFolderId()}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest bg-blue-600 text-white px-3 sm:px-4 py-2 rounded-lg hover:bg-blue-700 transition-all shadow-sm flex items-center gap-2"
                    >
                      <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M7.71,3.5L1.15,15L4.58,21L11.13,9.5L7.71,3.5Z M9.73,15L6.3,21H19.42L22.85,15H9.73Z M22.28,14L15.73,2.5H8.87L15.42,14H22.28Z" />
                      </svg>
                      Drive
                    </a>
                  )}
                  <button 
                    ...
                    className={`text-[9px] sm:text-[10px] font-bold uppercase tracking-widest px-3 sm:px-4 py-2 rounded-lg transition-all ${historyTab === 'batches' ? 'bg-black text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                  >
                    Batches
                  </button>
                  <button 
                    onClick={() => setHistoryTab('edits')}
                    className={`text-[9px] sm:text-[10px] font-bold uppercase tracking-widest px-3 sm:px-4 py-2 rounded-lg transition-all ${historyTab === 'edits' ? 'bg-black text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                  >
                    Edits
                  </button>
                  <button onClick={() => setShowMemory(false)} className="hidden sm:block p-2 hover:bg-gray-100 rounded-full transition-colors border border-gray-200">
                    <X className="w-5 h-5" />
                  </button>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-1 gap-6">
                    {batches.length === 0 ? (
                      <div className="text-center py-20 text-gray-400">
                        ...
                        <p className="text-sm font-bold uppercase tracking-widest">No batches found</p>
                      </div>
                    ) : (
                      batches.map((batch) => (
                        <div key={batch.id} className="bg-white rounded-[40px] p-8 shadow-sm hover:shadow-xl transition-all border border-gray-100 group">
                          <div className="flex flex-col md:flex-row gap-8">
                            {/* Grid Preview */}
                            <div className="w-full md:w-48 h-48 bg-gray-100 rounded-[30px] overflow-hidden grid grid-cols-2 gap-0.5 shrink-0">
                            ...
                            <div className="flex-1 flex flex-col justify-between py-1">
                              <div>
                                <div className="flex items-center justify-between mb-2">
                                  <h3 className="font-display font-black text-2xl tracking-tighter uppercase">{batch.title}</h3>
                                  <div className="flex gap-2">
                                    <button 
                                      className="p-3 bg-gray-100 rounded-2xl hover:bg-black hover:text-white transition-all shadow-sm group/btn"
                                      title="Zip & Download All"
                                      onClick={() => downloadBatch(batch)}
                                    >
                                      <Download size={18} className="group-hover/btn:scale-110 transition-transform" />
                                    </button>
                                    <button 
                                      className="p-3 bg-gray-100 rounded-2xl hover:bg-black hover:text-white transition-all shadow-sm"
                                      title="Open in Workbench"
                                      onClick={async () => {
                                        // Find images in this batch and load them back into the workbench
                                        ...
                                        for (const id of batch.imageIds) {
                                          ...
                                          if (m) {
                                            restoredImages.push({
                                              id: m.id,
                                              file: new File([], 'Restored File'), // Placeholder
                                ...
                                <div className="flex items-center gap-3 text-[10px] font-bold text-gray-400 mb-4 uppercase tracking-widest">
                                  ...
                                  {batch.address && (
                                    <>
                                      ...
                                      <MapPin size={12} />
                                      <span className="text-accent">{batch.address}</span>
                                    </>
                                  )}
                                </div>
                                
                                <p className="text-xs text-gray-400 font-medium italic line-clamp-2 leading-relaxed">&quot;{memory.find(m => m.id === batch.imageIds[0])?.prompt || 'No prompt info'}&quot;</p>
                              </div>
                              
                              <div className="flex gap-2 mt-4">
                                {batch.tags?.map(tag => (
                                  <span key={tag} className="text-[9px] font-black uppercase tracking-[0.2em] bg-gray-100 text-gray-500 px-3 py-1.5 rounded-xl">
                                    {tag}
                                  </span>
                                ))}
                                <span className="text-[9px] font-black uppercase tracking-[0.2em] bg-gray-100 text-gray-500 px-3 py-1.5 rounded-xl">
                                  {batch.imageIds.length} PHOTOS
                                </span>
                              </div>
                            </div>
                          </div>
                              ...
                              <div className="flex gap-3">
                                ...
                                <button 
                                  onClick={async () => {
                                    const full = await getFullMemoryItem(item.id);
                                    if (full?.editedImage) {
                                      saveAs(full.editedImage, `beautified_${item.id}.jpg`);
                                    } else {
                                ...
                                {(item.originalDriveId || item.editedDriveId) && (
                                  <a
                                    href={getDriveViewUrl(item.editedDriveId || item.originalDriveId!)}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="text-[10px] font-black uppercase text-green-600 hover:underline flex items-center gap-1"
                                  >
                                ...
                                <button 
                                  onClick={() => {
                                    ...
                                    if (item.settings?.model) setModel(item.settings.model);
                                  }}
              ...
              <div className="p-6 space-y-6 overflow-y-auto flex-1">
                ...
                <section className="flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className={`w-10 h-6 flex items-center rounded-full p-1 transition-colors ${isAnalysisEnabled ? 'bg-black' : 'bg-gray-200'}`}
                  >
                    <div className={`w-4 h-4 rounded-full bg-white transition-transform ${isAnalysisEnabled ? 'translate-x-4' : 'translate-x-0'}`} />
                  </button>
                </section>
                ...
                <section>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-text-muted">Creative Strength</label>
                    <span className="text-[10px] font-mono font-bold text-accent bg-accent/10 px-2 py-0.5 rounded-full">{Math.round(creativeStrength * 100)}%</span>
                  </div>
                  <input 
                    type="range" 
                    min="0.1" 
                    ...
                    onChange={(e) => setCreativeStrength(parseFloat(e.target.value))}
                    className="w-full h-1.5 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-accent"
                  />
                  <p className="mt-1 text-[8px] text-text-muted">Higher values allow the AI to be more transformative and creative.</p>
                </section>
              ...
              <div className="w-full md:w-96 p-8 flex flex-col gap-6 text-white border-l border-white/10 overflow-y-auto">
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                    ...
                    />
                    <button 
                      onClick={reprocessSingle}
                      disabled={isProcessing}
                      className="mt-2 w-full py-2 bg-white/10 hover:bg-white/20 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-2"
                    >
                      {isProcessing ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3 text-accent" />}
                      Run Again
                    </button>
                  </div>
                ...
                <div className="mt-auto pt-6">
                  <p className="text-[10px] text-gray-500 leading-relaxed">
                    AI-enhanced using Gemini 2.5 Flash Image. Slide to compare original vs beautified.
                  </p>
                </div>
              </div>
            </motion.div>
